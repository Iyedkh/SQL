...
-- Correcting the table name in the first constraint
ALTER TABLE EMPLOYEES  
ADD CONSTRAINT FK_EMPLOYEES_EMPLOYEES  -- Corrected constraint name
FOREIGN KEY (REPORTS_TO) REFERENCES EMPLOYEES(EMPLOYEE_NUMBER);

-- Removing unnecessary spaces in constraint names
ALTER TABLE PRODUCTS  
ADD CONSTRAINT FK_PRODUCTS_CATEGORIES  -- Corrected constraint name
...
SELECT 
    EMPLOYEE_NUMBER,
    LEFT(FIRST_NAME + SPACE(15), 15) AS FORMATTED_FIRST_NAME, 
    LEFT(LAST_NAME + SPACE(15), 15) AS FORMATTED_LAST_NAME,   
...
SELECT 
    ...
    CASE 
        WHEN od.ORDER_NUMBER BETWEEN 10000 AND 10999 THEN 'apply old discount rate'
...
HAVING COUNT(DISTINCT CASE WHEN cat.CATEGORY_NAME = 'Desserts' THEN od.PRODUCT_REF END) <= 1;
